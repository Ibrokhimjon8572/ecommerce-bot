...
class AddAddressHandler(Handler):
    ...
    def handle(self, msg, message_id=None):
        ...
        if type(msg) == types.Location:
            ...
            self.user_session.state = 'address_name'
            self.user_session.save()


class AddAddressDisplayer(Displayer):
    def __init__(self, control: Control):
        super().__init__(control)

    def show(self):
        self.reply(_("send location"), keyboards.add_address_keyboard())
